package com.ezcloud.framework.dao;
...
import com.ezcloud.framework.entity.OrderEntity;
import com.ezcloud.framework.page.orm.Filter;
import com.ezcloud.framework.page.orm.Filter.Operator;
import com.ezcloud.framework.page.orm.Order;
import com.ezcloud.framework.page.orm.Order.Direction;
 ...
 * JpaDao - 基类
 */
public abstract class JpaBaseDaoImpl<T, ID extends Serializable> implements JpaBaseDao<T, ID> {
	...
	private synchronized String getAlias(Selection<?> selection) {
		if (selection != null) {
			...
			if (alias == null) {
				...
				alias = "systemGeneratedAlias" + aliasCount++;
	...
	private void addRestrictions(CriteriaQuery<T> criteriaQuery, List<Filter> filters) {
		...
		for (Filter filter : filters) {
			...
			if (filter.getOperator() == Operator.eq && filter.getValue() != null) {
				...
				restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.greaterThan(root.<Timestamp> get(filter.getProperty()), (Timestamp) filter.getValue()));
			} else if (filter.getOperator() == Operator.lessThan && filter.getValue() != null) {
				restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.lessThan(root.<Timestamp> get(filter.getProperty()), (Timestamp) filter.getValue()));
			} else if (filter.getOperator() == Operator.greaterThanOrEqual && filter.getValue() != null) {
				restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.greaterThanOrEqualTo(root.<Timestamp> get(filter.getProperty()), (Timestamp) filter.getValue()));
			} else if (filter.getOperator() == Operator.lessThanOrEqual && filter.getValue() != null) {
				restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.lessThanOrEqualTo(root.<Timestamp> get(filter.getProperty()), (Timestamp) filter.getValue()));
