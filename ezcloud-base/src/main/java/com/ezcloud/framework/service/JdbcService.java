...
public class JdbcService {
	...
	// 更新
	public int update(Row row,String tableName,String keyName,String updateFieldName ) {
		int rowNum =0;
		String id=row.getString(keyName);
		if ( StringUtil.isEmptyOrNull( updateFieldName ) ) {
			rowNum =update( row, tableName, keyName );
		}
		else
		{
			row.put(updateFieldName, DateUtil.getCurrentDateTime());
			rowNum =update(tableName, row, keyName+" ='"+id+"'");
		}
		return rowNum;
	}
	
	// 更新
	public int update(Row row,String tableName,String keyName ) {
		int rowNum =0;
		String id=row.getString(keyName);
	...
	// 新增
	public int insert(Row row,String tableName,String keyName) {
		int rowNum = 0;
		int id =getTableSequence(tableName, keyName, 1);
		row.put(keyName, id);
		row.put("create_time", DateUtil.getCurrentDateTime());
		rowNum =insert(tableName, row);
		return rowNum;
	}
	
	// 新增
	public int insert(Row row,String tableName,String keyName,String createFieldName) {
		int rowNum = 0;
		int id =getTableSequence(tableName, keyName, 1);
		row.put(keyName, id);
		if (StringUtil.isEmptyOrNull( createFieldName )) {
			rowNum =insert( row, tableName, keyName);
		}
		else
		{
			row.put(createFieldName, DateUtil.getCurrentDateTime());
		}
		rowNum =insert(tableName, row);
		return rowNum;
	}
