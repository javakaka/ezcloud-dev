	...
	<form id="listForm" action="list.do" method="get">
		...
		<table id="listTable" class="list">
			<tr>
				...
				<th>
					<a href="javascript:;" class="sort" name="RECEIVE_NAME">收货人姓名</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="RECEIVE_TEL">收货人联系电话</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="IS_DEFAULT">默认地址</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="ZIP_CODE">邮编</a>
				</th>
				<th>
			...
			<c:forEach items="${page.content}" var="row" varStatus="status">
				<tr>
					...
					<td>
						<span title="${row.RECEIVE_NAME}">${row.RECEIVE_NAME}</span>
					</td>
					<td>
						<span title="${row.RECEIVE_TEL}">${row.RECEIVE_TEL}</span>
					</td>
					<td>
						<c:choose>
							<c:when test="${row.IS_DEFAULT == 1}">是</c:when>
							<c:when test="${row.IS_DEFAULT == 0}">否</c:when>
							<c:otherwise>--</c:otherwise>
						</c:choose>
					</td>
					<td>
						<span title="${row.ZIP_CODE}">${row.ZIP_CODE}</span>
					</td>
					<td>
						${row.PROVINCE_NAME}
					</td>
