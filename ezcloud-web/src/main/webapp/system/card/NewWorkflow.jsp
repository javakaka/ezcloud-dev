...
<base target="_self">
<%
   int iOperate =Integer.parseInt(ServletUtil.get(request, "operate", "1"));
...
//					out.println(sFieldEname);
                  ...
                  out.println("");
               else if(sStyleId.equals("005002"))//单项选择(radio)
					{
						...
						while(enTmp.hasMoreElements()){
							...
							Object oPara =enTmp.nextElement();
							hmDataTmp.put(oPara, request.getParameter(oPara.toString()));
						}
					%>
                  <$:A service="getSourceData" dynamic="false" condition="<%=hmDataTmp%>">
                     <$:P name="table_ename" value="<%=sCard%>"/>
                     <$:P name="field_ename" value="<%=sFieldEname%>"/>
                  </$:A>
                  <$:M dtype="1"/>
               ...
               else if(sStyleId.equals("005004"))//数据下拉
               {
					...
					%>
                  <$:A service="getSourceData" dynamic="false" condition="<%=hmDataTmp%>">                     	<$:P name="table_ename" value="<%=sCard%>"/>
                     <$:P name="field_ename" value="<%=sFieldEname%>"/>
                  </$:A>
                  <$:M dtype="1"/>
...
<input type=hidden name="operate" value="<%=iOperate%>">
<%
//网页传传过来的参数
...
function onEdit(form)
{
   ...
   if(checkForm(form))
   {
		...
		<%=sOnSaveJs%>
		var sWfNo =$E.getParameter("wf_no");
		if(sWfNo !=null && sWfNo !="")
			$S("operate", "2");//如果流程编号不为空，则置为编辑状态
		var s=new Server("saveFpuCardData");
		s.setFormData();
		$E.disabled();
		//*
		s.send(s, false, function(){
				alert("保存成功！");
				//alert(s.getXml());
				var oResult =s.getResult("ReturnData");
				if($V("operate") =="1" && oResult !=null)
				{//如果是添加，作页面跳转
					/*
					var sWfNo =oResult.get(1, "WF_NO");
					var sFpuNo =oResult.get(1, "FPU_NO");
					var sProcessNo =oResult.get(1, "PROCESS_NO");
					var sCard =oResult.get(1, "CARD");
					var sUrl ="/oa/task/ActionTask.jsp?wf_no="+sWfNo
						+"&fpu_no="+sFpuNo+"&process_no="+sProcessNo
						+"&card="+sCard;
					window.location =WEB_PATH+sUrl;
					*/
					var saDatas =oResult.joint("&");
					var sUrl ="/oa/task/ActionTask.jsp?1=1";
					for(k=0; k<saDatas.length; k++)
					{
						sUrl +="&"+saDatas[k];
					}
					window.location =WEB_PATH+sUrl;
...
<$:A service="getFpuCanOperateFields" dynamic="false">
   <$:P name="wf_no" from="request"/>
   <$:P name="swf_no" from="request"/>
   <$:P name="fpu_no" from="request"/>
</$:A>
<$:M dtype="1"/>
<$:T var="fields" name="Data">
   <script>
      var field ='<$:C item="fields.CANOPERATEFIELDS"/>';
      fields =field.split(",");
      ...
      else
      {
         for(i=0; i<fields.length; i++)
         {
            ...
            {
               if(document.all[j].name ==fields[i])
               {
                  document.all[j].disabled =true;
						try
						{
							document.getElementsByName(document.all[j].name+"IMG")[0].style.display ="none";
						}
						catch(e){}
						document.all[j].className="read";
               }
            }
         }
      }
