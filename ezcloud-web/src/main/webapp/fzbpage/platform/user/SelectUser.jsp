	...
	<form id="listForm" action="SelectUser.do" method="get">
			...
			<div class="menuWrap">
				...
				<div class="popupMenu">
					<ul id="searchPropertyOption">
						...
						<li>
							<a href="javascript:;" <c:if test="${page.searchProperty == 'EMAIL'}">class="current"</c:if> val="EMAIL">邮箱</a>
						</li>
						<li>
		...
		<table id="listTable" class="list">
			...
			<c:forEach items="${page.content}" var="row" varStatus="status">
				<tr>
					<td>
						<input type="radio" name="ids" value="${row.ID}" title="${row.TELEPHONE}" <c:if test="${id == row.ID}">checked</c:if>/>
					</td>
					<td>
						${row.TELEPHONE}
					</td>
