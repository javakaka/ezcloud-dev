...
<body>
	...
	<form id="listForm" action="CustomerList.do" method="get">
		<div class="bar">
			<!--
			...
			<div class="menuWrap">
				...
				<div class="popupMenu">
					<ul id="searchPropertyOption">
						<li>
							<a href="javascript:;" <c:if test="${page.searchProperty == 'AGENT_NAME'}"> class="current"</c:if> val="AGENT_NAME">中介姓名</a>
						</li>
						<li>
							<a href="javascript:;" <c:if test="${page.searchProperty == 'AGENT_TELEPHONE'}">class="current"</c:if> val="AGENT_TELEPHONE">中介手机</a>
						</li>
					</ul>
				</div>
			</div>
		...
		<table id="listTable" class="list">
			...
			<!--
				...
				-->
				<th>
					<a href="javascript:;" class="sort" name="AGENT_NAME">中介姓名</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="AGENT_TELEPHONE">中介手机</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="CODE">房源编号</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="ROOM_STATUS">房源状态</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="LANDLORD_NAME">房东姓名</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="LANDLORD_TELEPHONE">房东手机</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="RENT_NAME">租客姓名</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="RENT_PHONE">租客手机</a>
				</th>
				<!--
				<th>
			...
			<c:forEach items="${page.content}" var="row" varStatus="status">
				...
				<!--
					...
					-->
					<td>
						<span title="${row.AGENT_NAME}">${row.AGENT_NAME}</span>
					</td>
					<td>
						<span title="${row.AGENT_TELEPHONE}">${row.AGENT_TELEPHONE}</span>
					</td>
					<td>
					...
					<td>
						<c:choose>
							<c:when test="${row.ROOM_STATUS == '2' || row.ROOM_STATUS == '-1'}">出租中</c:when>
							<c:when test="${row.ROOM_STATUS == '-2' || row.ROOM_STATUS == '-3'}">已结束</c:when>
					...
					<td>
						<span title="${row.RENT_PHONE}">${row.RENT_PHONE}</span>
					</td>
					<!--
					<td>
